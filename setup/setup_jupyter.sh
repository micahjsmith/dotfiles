...
if command -v jupyter >/dev/null 2>&1; then
    ...
    if [ ! -d "${nbextensionsdir}/vim_binding" ]; then
    ...
    if ! jupyter nbextension list 2>/dev/null | grep -q vim_binding; then
        jupyter nbextension enable vim_binding/vim_binding
        echo "$SCRIPTNAME: enabled vim_binding/vim_binding"
    fi

    # If not linked, link custom.js.
    if [ ! -L "$HOME/.jupyter/custom/custom.js" ]; then
